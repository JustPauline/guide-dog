...
class DOGZILLA():
    ...
    def __read(self, addr, read_len=1):
        ...
        self.ser.flushInput()
        self.ser.write(tx)
        # print(f"[TX] Read request to 0x{addr:02X}, length {read_len} → {tx}")
        if self.verbose:
            print("tx_data: ", tx)
    ...
    def decode_motor_angles(self, raw_bytes):
        def decode(byte, limit):  # limit = [min, max]
            return ((byte / 255.0) * (limit[1] - limit[0])) + limit[0]

        joints = []
        for i in range(0, 9, 3):
            calf = round(decode(raw_bytes[i],   [-73, 57]), 1)
            thigh = round(decode(raw_bytes[i+1], [-66, 93]), 1)
            shoulder = round(decode(raw_bytes[i+2], [-31, 31]), 1)
            joints.append((thigh, calf, shoulder))

        angles = [joints[1][0], joints[1][1], joints[1][2], joints[2][0], joints[2][1], joints[2][2], joints[0][0], joints[0][1], joints[0][2], joints[1][0], joints[1][1], joints[1][2]]
    ...
    def read_motor(self, out_int=False):
        ...
        # print("[Motor Read] Raw data:", [hex(b) for b in raw])

        try:
            joint_bytes = raw[:9]
            angles = self.decode_motor_angles(joint_bytes)
        except Exception as e:
            print("[Motor Read] Error decoding:", e)
            angles = []
        return angles
    ...
    def read_imu_raw(self):
        """
        Read IMU raw data from serial, return:
        ...
        if len(raw) < 28:
            self.get_logger().warn("IMU packet too short")
            return None
        if raw[0] != 0x55 or raw[1] != 0x00 or raw[-1] != 0xAA:
            self.get_logger().warn("Invalid IMU packet header/footer")
            return None
        # print(raw)
        data = raw[2:-1]  # Strip header/footer
        # print(data)
        imu_data = []

        # Parse acc (first 3 int16), gyro (next 3 int16)
        for i in range(6):
            val = struct.unpack("<h", data[i*2:i*2+2])[0]
            if i < 3:
                imu_data.append(val / 16384.0 * 9.8)  # accelerometer in m/s²
            else:
                imu_data.append(val / 16.4)  # gyroscope in deg/s (assumed)

        # Parse roll, pitch, yaw (3 x float32) — convert to radians
        for i in range(3):
            start = 12 + i * 4
            val = struct.unpack("<f", data[start:start+4])[0]
            imu_data.append(val * math.pi / 180.0)  # radians

        # Optional debug prints:
        print(f"ACC:  {imu_data[:3]}")
        # print(f"GYRO: {imu_data[3:6]}")
        # print(f"RPY:  {imu_data[6:]}")
